...
| Published `ASSAULT\_2025` | 24,686 | | |
...
\- \*\*Data model:\*\* a many-to-one relationship on the numeric neighbourhood ID `HOOD\_158`, joining offences to 158 neighbourhoods. The ID is zero-padded in one source and not the other, so it is typed as a whole number on both sides.

\- \*\*DAX:\*\* four measures, including a share-of-total using `CALCULATE` and `ALL`. See \[`docs/dax\_measures.md`](docs/dax\_measures.md). The model table is named `Incidents`; every row in it is an offence.
...
| `workbook/toronto\_crime\_analysis.xlsx` | The workbook. Re-point the queries before refreshing: see `docs/DATA\_SOURCE.md` |

| `docs/toronto\_crime\_analysis.pdf` | Dashboard, forecast and reconciliation, readable without Excel |

| `docs/limitations.md` | Every known limitation, in one place |

| `docs/DATA\_SOURCE.md` | Where the data comes from and how to reproduce it |

| `queries/`, `docs/dax\_measures.md` | The Power Query and DAX code |
